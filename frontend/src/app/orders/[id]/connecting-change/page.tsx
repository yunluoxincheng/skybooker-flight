...
export default function ConnectingChangePage() {
  ...
  const load=useCallback(async()=>{try{const [o,list]=await Promise.all([orderApi.getOrderById(id),orderApi.getConnectingChangeOptions(id)]);if(o.journeyType!=="CONNECTING"){router.replace(`/orders/${id}/change`);return}setOrder(o);setOptions(list)}catch(e){setError((e as ApiError).message||"加载整段改签方案失败")}},[id,router])
  ...
  return <UserLayout><main className="mx-auto max-w-5xl space-y-6 px-4 py-8"><div><Badge className="mb-2 bg-sky-700">整段联程改签</Badge><h1 className="text-2xl font-bold">选择完整替代行程</h1><p className="text-sm text-muted-foreground">系统会先原子锁定新行程的全部座位，再释放原行程；不支持只改其中一段。</p></div>{error&&<div className="rounded-lg bg-destructive/10 p-3 text-sm text-destructive">{error}</div>}
