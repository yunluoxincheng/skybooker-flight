...
export function JourneyCard({ journey, cabinClass }: { journey: ItineraryVO; cabinClass?: CabinClass }) {
  ...
  const soldOut = journey.availableSeats <= 0
  const actionUrl = soldOut && !connecting ? waitlistUrl(journey, cabinClass)! : detailUrl
  const actionLabel = soldOut && !connecting ? "立即候补" : "查看详情"
  return <Card className="transition-shadow hover:shadow-md"><CardContent className="p-5">
    <div className="flex flex-col gap-4 lg:flex-row lg:items-center">
      ...
      <div className="flex shrink-0 items-center justify-end gap-4 lg:w-[200px]"><div className="text-right"><FlightPriceTag price={journey.estimatedAmount} className="text-lg"/><p className={`text-xs ${soldOut ? "text-destructive" : "text-muted-foreground"}`}>{soldOut ? "已售罄" : `每人起 · 余 ${journey.availableSeats} 座`}</p></div><Button render={<Link href={actionUrl}>{actionLabel}</Link>} size="sm" nativeButton={false}/></div>
    </div>
