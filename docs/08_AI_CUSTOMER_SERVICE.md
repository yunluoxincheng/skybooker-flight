...
多轮 continuation 只在上一条助手消息是 `FOLLOW_UP` 且带 `missingFields` 时触发，依据“当前回复是否在补全航班查询条件”判定（含航班信号、parser 解析出任一查询字段、或文本是已知城市名/乘客数等裸补全词），不使用“短回复长度”启发式。进入 continuation 后，即使当前回复已补齐必填字段，也会继承上一轮未被当前轮覆盖的可选筛选条件（如乘客数、舱位、航司、价格、时段、直飞、排序）。
