...
- `FLIGHT_QUERY` / `FLIGHT_QUERY_CONTINUATION`：涉及机票、航班、票价、余票、舱位、时刻等系统事实，进入条件解析、上下文合并和数据库推荐链路；
- `BOOKING_HELP`：订票、退票、改签、候补、乘机人、订单、账号等 SkyBooker 固定流程说明，返回 `BOOKING_HELP`，使用后端固定模板；
- `OUT_OF_SCOPE`：拒绝非旅行、非航班、非 SkyBooker 平台帮助请求。

多轮 continuation 只在上一条助手消息是 `FOLLOW_UP` 且带 `missingFields` 时触发，依据“当前回复是否在补全航班查询条件”判定（含航班信号、parser 解析出任一查询字段、或文本是已知城市名/乘客数等裸补全词），不使用“短回复长度”启发式。
