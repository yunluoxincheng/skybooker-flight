...
- `GREETING`：问候和能力介绍，返回 `CHAT_REPLY`；
- `TRAVEL_ADVICE`：目的地玩法、行程和出行准备建议，返回 `TRAVEL_ADVICE`，不输出具体航班事实；
- `PLATFORM_HELP`：订票、退票、改签、候补、乘机人、订单、账号等 SkyBooker 固定流程说明，返回 `PLATFORM_HELP`；
- `FLIGHT_SEARCH` / `FLIGHT_SEARCH_CONTINUATION`：进入条件解析、上下文合并和数据库推荐链路；
