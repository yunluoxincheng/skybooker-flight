...
| Playwright 浏览器验证 `/admin/flights` | 已确认后台无航司/机场管理菜单，新增航班只填写航司 ID 和机场 ID，前端 Issue #91 |
| `curl /api/admin/airlines`、`curl /api/admin/airports` | 均返回 404 `资源不存在`，后端 Issue #90 |
| GitHub issue 创建 | 已追加 #87、#88、#89、#90、#91；#89 拆分为 #90/#91 并关闭 |
...
| 航司/机场管理 | `/admin/flights`、`/api/admin/airlines`、`/api/admin/airports` | 后台无管理入口，接口返回 404，新增航班依赖手工 ID | 未通过，后端 #90，前端 #91 |
...
| KI-003 | 全站表单校验错误不展示且提交状态异常 | S1 | 用户指定 `localhost:3000` 抽样通过；`127.0.0.1` dev origin 环境复现 | #81/#96 |
| KI-004 | 默认列表过期航班仍可预订，提交订单返回 400“航班不可预订” | S0 | 已复现，范围已澄清 | #82 |
| KI-005 | 密码可见性按钮在特定 origin 下点击无效 | S3 | 用户指定 `localhost:3000` 通过，`127.0.0.1` 随 #96 复现 | #83/#96 |
...
- 表单错误处理：用户指定 `localhost:3000` 下，公开认证表单、预订/候补新增乘机人、管理端新增航班抽样均通过；`127.0.0.1` 访问 Next dev 会触发 origin 限制并导致客户端事件未接管，需按 #96 明确开发访问地址或配置 `allowedDevOrigins`。
