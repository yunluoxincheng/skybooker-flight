...
管理端分页列表统一使用 `page >= 1`、`1 <= size <= 100`，默认 `page=1&size=10`。搜索/筛选字符串参数会先 `trim()`；`""` 与纯空白字符串按未传条件处理，非空值以 trim 后的值查询。非法分页、非法枚举、非法日期格式或数字类型转换失败统一返回 HTTP 400 + `VALIDATION_ERROR (10003)`。
...
GET    /api/admin/connecting-itineraries        # 分页 JOIN 摘要：?page&size；含两段/联程余票、sellable 与 unavailableReason
...
`GET /api/admin/orders` 支持后台筛选条件：`status` 精确匹配订单状态，`orderNo` 模糊匹配订单号，`userId` 精确匹配用户，`userKeyword` 模糊匹配邮箱、昵称、姓名或手机号，`flightNo` 模糊匹配航班号，`flightKeyword` 模糊匹配航班号、航司、出发/到达城市或机场名，`departureDateStart`/`departureDateEnd` 按航班出发日期范围过滤。筛选条件必须同时作用于 `records` 和 `total`；非法状态、日期或分页参数返回 `VALIDATION_ERROR`，不得静默返回未筛选列表。
