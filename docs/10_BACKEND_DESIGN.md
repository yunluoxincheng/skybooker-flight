...
- 支付、取消、退票、候补支付、候补退款和改签都必须校验当前状态，重复请求要返回当前最终结果，不能重复扣减、释放或退款。
- 候补支付成功时只进入 `WAITING` 排队，不创建正式订单；候补兑现成功时必须创建状态为 `ISSUED` 的正式订单，并把正式订单 ID 写回 `waitlist_order.ticket_order_id`。
