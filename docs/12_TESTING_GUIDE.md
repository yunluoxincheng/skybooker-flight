...
- 搜索边界：同机场 90 分钟、6 小时和跨日组合有效；89 分钟、超过 6 小时、断开的机场、已起飞、未上架、取消和余票不足均排除。
- 下单一致性：两段乘机人集合必须一致；第二段锁座或扣库存失败时，父订单、两段快照、座位锁和余票计数均回滚。
- 生命周期：一次支付售出全部座位；取消/过期/退票释放全部座位并分别回补每段库存；重复请求和并发状态 CAS 不重复产生副作用。
- 改签：覆盖联程到直飞、联程到联程，验证先保护新方案后释放旧方案，并拒绝单段替换、错误起终点和首段起飞后的操作。
- 前端：运行 `pnpm exec tsc --noEmit`、`pnpm lint`、`pnpm build`，人工确认中转卡、逐段选座和订单多航段展示。
...
期望：返回 `TRAVEL_CHAT`，不要因为“多少钱/预算”进入航班查询。
...
期望：前者返回 `FOLLOW_UP` 追问出发日期；后者进入 `FLIGHT_QUERY` 查询链路。
...
- 多轮补全时，最新 assistant `FOLLOW_UP` 且有 `missingFields` 时，短回复如“明天”必须优先按 `FLIGHT_QUERY_CONTINUATION` 合并上下文（判定依据是“当前回复是否在补全航班查询条件”，不使用文本长度启发式）；即使当前轮已补齐必填条件，也要继承上一轮未被覆盖的可选筛选字段，如舱位、人数、航司、价格、时段、直飞和排序；如果上一条 assistant 不是 `FOLLOW_UP` 缺槽回复，新的完整查询不得继承上一轮已完成查询的可选筛选条件；
