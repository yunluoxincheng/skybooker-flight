...
| profile | 用途 | 当前规模 |
|---|---|---|
| dev | 本地开发、课堂演示、小规模手工测试 | 完整 297 个机场参考资料；24 个机场产生航班、13 家航司、90 条航线组合、未来 7 天、32 个用户、120 个订单 |
| test | 功能测试、集成测试、报表和查询验证 | 完整 297 个机场产生航班、24 家航司、至少 435 条航线组合、未来 30 天、240 个用户、2200 个订单 |
| perf | 性能测试扩展入口 | 完整 297 个机场产生航班、36 家航司、至少 1200 条航线组合；文件较大，建议在压测环境本地生成后导入 |
...
基础航司和机场通过 `code` 幂等 upsert，不使用清理区间删除，便于 dev/test 共用基础航空资料。机场目录位于 `scripts/data/airports-cn.json` 和 `scripts/data/airports-international.json`：当前包含 260 个中国大陆机场、15 个港澳台机场和 22 个国际枢纽；目录元信息记录了 OurAirports 快照来源和更新时间，具体口径见 `scripts/data/README.md`。港澳台单独标记为 `special_region`，不混入中国大陆机场统计。
