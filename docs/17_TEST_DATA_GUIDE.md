...
`seed` 会自动替换同一 profile 的旧 ownership 数据，不需要先执行 `clean`：
...
只删除脚本生成的数据时使用 `clean`；它按 V18 ownership 删除关联行并保留航司、机场、Flyway 历史和手工数据：

```bash
./scripts/test-data.sh clean --dir "$PWD" --source-dir "$PWD" \
  --profile dev --yes
```

`docker compose down -v` 是删除整个本地数据库 volume 的破坏性操作，会同时删除手工数据和 Flyway 历史；只有明确需要重建本地数据库时才使用它。

### 已部署服务器：固定版本刷新

服务器没有仓库克隆时，下载入口并固定到已审查的 commit SHA。推荐流程仍是 `doctor → status → seed → validate --database → status`：
...
bash /tmp/skybooker-test-data.sh doctor \
  --dir /opt/skybooker --repo yunluoxincheng/skybooker-flight --ref <commit-sha>
bash /tmp/skybooker-test-data.sh status \
  --dir /opt/skybooker --repo yunluoxincheng/skybooker-flight --ref <commit-sha>
bash /tmp/skybooker-test-data.sh seed \
  --dir /opt/skybooker --repo yunluoxincheng/skybooker-flight --ref <commit-sha> \
  --profile dev --base-date "$(date +%F)" --yes
...
生产标记环境还必须显式传 `--allow-production --confirm-production --yes`。

## 1. 目标

SkyBooker 的测试数据采用“真实基础数据 + 规则生成业务数据”：

- 机场和航空公司使用独立、可审查的静态目录，包含稳定 IATA 代码、城市、省级行政区/国家或地区和数据来源元信息；
...
./scripts/test-data.sh seed --profile dev --base-date "$(date +%F)" --scenarios all --yes
...
```bash
curl -fsSL https://raw.githubusercontent.com/yunluoxincheng/skybooker-flight/<commit-sha>/scripts/test-data.sh \
  -o /tmp/skybooker-test-data.sh
bash /tmp/skybooker-test-data.sh status \
  --dir /opt/skybooker \
...
| profile | 用途 | 当前规模 |
|---|---|---|
| dev | 本地开发、课堂演示、小规模手工测试 | 完整 311 个机场参考资料；24 个机场产生航班、13 家航司、90 条航线组合、未来 7 天、32 个用户、120 个订单 |
| test | 功能测试、集成测试、报表和查询验证 | 完整 311 个机场产生航班、24 家航司、至少 435 条航线组合、未来 30 天、240 个用户、2200 个订单 |
| perf | 性能测试扩展入口 | 完整 311 个机场产生航班、36 家航司、至少 1200 条航线组合；文件较大，建议在压测环境本地生成后导入 |
...
基础航司和机场通过 `code` 幂等 upsert，不使用清理区间删除，便于 dev/test 共用基础航空资料。机场目录位于 `scripts/data/airports-cn.json` 和 `scripts/data/airports-international.json`：当前包含民航局 2025 年底口径的 270 个中国大陆颁证运输机场、15 个港澳台机场和 26 个国际枢纽，共 311 个；CI 校验大陆机场精确数量和 IATA 代码集合 SHA-256。港澳台单独标记为 `special_region`，不混入中国大陆机场统计，具体来源见 `scripts/data/README.md`。

目录升级只处理生成器中明确登记的旧 seed 代码（当前为 `WEN`、`DAX`）：没有航班引用时删除，仍被航班引用时改为 `DISABLED`。脚本不会删除其他目录外机场，因此手工维护的机场会保留。数据库校验会确认 311 个批准机场的字段指纹一致，并确认已知旧代码不存在或已禁用。

profile 的 `flight_airport_count` 只控制哪些机场生成航班，不再截断参考机场目录。航线生成先建立每个选中机场一进一出的覆盖骨架，再补充主要国内枢纽双向航线、国际/特殊地区到中国大陆枢纽的连接，最后按 profile 目标随机补充。

## 4. 生成数据

```bash
# 生成开发数据（推荐统一入口）
./scripts/test-data.sh generate --profile dev --seed 20260707 --base-date "$(date +%F)" --scenarios all
...
统一入口 `test-data.sh generate/seed` 未传 `--base-date` 时默认使用当天日期，也可像上例显式传入以便审计。直接调用底层 `generate_test_data.py` 时仍由 seed 推导日期，以保持可复现性；CI 和固定测试必须显式传固定 `--base-date`。
...
- IATA 代码唯一、参考机场目录分区计数和数据来源元信息是否完整；
- 选中航班机场是否都有进港和出港航班，国际/特殊地区机场是否连接中国大陆枢纽，主要国内枢纽是否有双向航线；
...
- 机场和航司目录已拆为 `scripts/data/*.json`；后续只需按来源快照更新目录元信息并重新跑覆盖矩阵；
