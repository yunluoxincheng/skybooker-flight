...
后端只从当前 USER portal JWT principal 读取用户身份，不接收前端传入的 `userId`。注销前必须提交当前密码做二次确认；确认失败计入登录失败限流，避免登录态被盗后暴力尝试密码。

注销前检查账号是否仍有未完成订单、进行中候补、处理中退票或改签。存在任一活跃业务时拒绝注销，并要求用户先完成或处理对应业务。注销接口不自动取消候补、不触发候补退款、不取消订单、不修改退票或改签流程。
...
- 通过 refresh token 版本号吊销该用户全部用户端会话，旧 access token 也会在认证过滤器中失效。
