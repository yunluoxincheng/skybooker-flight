...
- **优先级**：P2
- **现象**：后端 AI 助手不再把所有非完整航班查询都当作 `FOLLOW_UP`。旅游闲聊（含问候）、平台帮助和越界问题会返回新的 `replyType` 与 `intent`，当前前端若只识别旧类型，可能出现展示样式不准、误渲染缺失字段、快捷操作行为不一致或重复展示追问文本。
- **后端变更 / 证据**：
  - 非搜索 `replyType`：`TRAVEL_CHAT`、`BOOKING_HELP`、`OUT_OF_SCOPE`。
  - 搜索链路 `replyType`：`FOLLOW_UP`、`FLIGHT_RECOMMENDATION`、`NO_RESULT`。
  - `intent` 元数据：`TRAVEL_CHAT`、`FLIGHT_QUERY`、`FLIGHT_QUERY_CONTINUATION`、`BOOKING_HELP`、`OUT_OF_SCOPE`。
...
- **建议修复**：
  ...
  2. `TRAVEL_CHAT`、`BOOKING_HELP`、`OUT_OF_SCOPE` 渲染为普通助手文本，不展示缺失字段 UI，不渲染航班卡片。
...
- **验证**：
  ...
  - 输入"退票怎么操作"：显示平台帮助文本。
  - 输入"帮我写代码"：显示越界拒绝文本。
  - 输入不完整航班搜索后再回复"明天"：仍能按 `FLIGHT_QUERY_CONTINUATION` 合并上下文。
