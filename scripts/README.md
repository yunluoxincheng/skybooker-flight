...
./scripts/test-data.sh seed --dir "$PWD" --source-dir "$PWD" --profile dev --base-date "$(date +%F)" --yes
...
bash /tmp/skybooker-test-data.sh seed \
  ...
  --ref <commit-sha> --profile dev --base-date "$(date +%F)" --yes
...
- `data/`：可审查的机场和航司基础目录；机场当前包含民航局口径的 270 个中国大陆颁证运输机场、15 个港澳台机场和 26 个国际枢纽，并在 JSON metadata 中记录来源、快照日期和精确集合校验值。
...
`refresh-demo-flight-dates.sql` 现在只保留为兼容占位。Flyway 不再自动插入演示航班；统一入口的 `generate/seed` 默认以当天为基准，底层生成器的可复现任务应显式传 `--base-date`。
