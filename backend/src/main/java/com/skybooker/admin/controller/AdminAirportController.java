...
public class AdminAirportController {
    ...
    @GetMapping
    public ApiResponse<PageResponse<AirportVO>> list(
            @RequestParam(required = false) String keyword,
            @RequestParam(required = false) String status,
            @RequestParam(defaultValue = "1") int page,
            @RequestParam(defaultValue = "10") int size) {
        return ApiResponse.success(adminAirportService.listAirports(keyword, status, page, size));
    }
    ...
    @PutMapping("/{id}")
    public ApiResponse<AirportVO> update(@PathVariable Long id, @Valid @RequestBody AirportDTO dto) {
